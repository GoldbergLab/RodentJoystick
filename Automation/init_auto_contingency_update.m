...
auto_contingency_timer.TimerFcn = @(~,~)regular_contingency_updates(handles);
...
function regular_contingency_updates(handles)
    update_all_boxes_anlys_gui(handles);
    [handles, failures, attachments] = write_out_all_contingencies_anlys_gui(handles, 0);
end
