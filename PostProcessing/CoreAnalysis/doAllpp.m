...
if ~failedflag && ((analysisflag<=2 && ~singlestep) || analysisflag == 2)
    try
        fileformatspec = '%f %f %s %s %s %s %s %s %s %s'; numfield = 10;
...
%% Update Contingency for Automated Training
if ~failedflag && ((analysisflag<=5 && ~singlestep) || analysisflag == 5)
    try
        update_contingency(working_dir);
    catch e
        failedflag = 5; err = getReport(e);
    end
end
...
%% Create PPT report
if ~failedflag && ((analysisflag<=5 && ~singlestep) || analysisflag == 6)
    try
        create_ppt_report(dirlist);
    catch e
        close all; % Close all open figures generated by create ppt
        failedflag = 6; err = getReport(e);
    end
end
