...
%   i
    ...
    out_thresh(i) = str2num(contingency_angle{end-7});
    hold_time(i) = str2num(contingency_angle{end-6});
    hold_thresh(i) = str2num(contingency_angle{end-5});
    angle1(i) = str2num(contingency_angle{end-4});
    angle2(i) = str2num(contingency_angle{end-3});
...
%     [~,dur_nl_t,dur_nl_t_r] = posthreshcross(stats_nl,dist*(6.35/100),0,0,1,10,[],0);
      [~,dur_l_t,dur_l_t_r,~] = tau_theta(stats,hold_thresh(i)*(6.35/100),out_thresh(i)*(6.35/100),1,0,[],0);
      [~,dur_nl_t,dur_nl_t_r,~] = tau_theta(stats,hold_thresh(i)*(6.35/100),out_thresh(i)*(6.35/100),2,0,[],0); 
    catch
    end
