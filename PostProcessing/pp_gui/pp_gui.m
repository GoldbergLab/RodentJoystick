...
helptxt = {['For more detailed information on how to use the GUI and details about each function, see the wiki: ', ...
    'https://github.com/GoldbergLab/RodentJoystick/wiki'], ...
    '',...
    '1.Select directories representing days. "Combine days" combines all data into a single plot.', ...
    '2. Select a plot for an axes via the drop down menu.',...
    '3. Change the argument values, if desired.',...
    '',...
    '',...
    'Argument Info:',...
    '',...
    ...
    '',...
    'Normalize (Hold Length Distributions, Nosepoke/Reward Activity): Flag (zero/nonzero) that normalizes raw counts to probability distribution if on', ...
    '',...
    'Thresh (Hold Length Distribution (Threshold)): computes hold times for trajectories under thresh', ...
    '',...
    ...
    'Save Plots:',...
    '',...
    'Select a layout: single plots all subplots as is (temporarily rendering menus/toolbars invisible) - but requires the export_figs package, separate generates 6 individual figures',...
    'Select a format: .fig only supported for saving separate figures, or .png',...
    '',...
